...
import static com.pixelmed.database.DicomDatabaseInformationModel.getQuotedEscapedSingleStringValueOrNull;
...
public abstract class DatabaseInformationModel {
	 ...
	 */
  public InformationEntity getRootInformationEntity() { return rootInformationEntity; }
	...
	public String insertObject(AttributeList list,String fileName,String fileReferenceType, String selectHeader, String insertHeader, InformationEntity headerType) throws DicomException {
		...
		try {
			...
			while (ie != null) {
				...
				if (selectHeader != null && headerType != null && ie == headerType) {
					b.append(selectHeader);
				} else {
...
//slf4jlogger.debug("Select all values for all records of {} with matching parent time {} milliseconds",ieWanted.toString(),(System.currentTimeMillis()-startTime));
	...
	public String getStatementStringWithMatchingAttributesForSelectedInformationEntity(AttributeList list, InformationEntity ie) throws DicomException {
		StringBuffer b = new StringBuffer();
...
//System.err.println("PatientStudySeriesConcatenationInstanceModel.extendStatementStringWithMatchingAttributesForSelectedInformationEntity(): patientName now "+patientName);
	...
	public Map<String, String> getInsertStatementMapWithAttributeValuesForSelectedInformationEntity(AttributeList list, InformationEntity ie, String fileName, String fileReferenceType) throws DicomException{
		...
		while (i.hasNext()) {
			...
			if (ie == dictionary.getInformationEntityFromTag(tag)) {
				...
				if (columnName != null && columnType != null && isAttributeUsedInTable(tableName,columnName)
					...
					attributes.put(columnName, getQuotedEscapedSingleStringValueOrNull(a));
		...
		if (ie == InformationEntity.INSTANCE) {
			...
			if (sopClassUID != null && SOPClass.isImageStorage(sopClassUID)) {
				...
				if (aImageOrientationPatient != null && aImageOrientationPatient.getVM() == 6) {
					...
					if (columnName != null && columnType != null && isAttributeUsedInTable(tableName,columnName)
							&& !whatHasBeenAdded.contains(columnName)) {								// do not add same attribute twice to same table
						attributes.put(columnName, getQuotedEscapedSingleStringValueOrNull(aImageOrientationPatient));
						whatHasBeenAdded.add(columnName);
					}
				}
			}
	...
	public String getInsertStatementStringWithAttributeValuesForSelectedInformationEntity(AttributeList list, InformationEntity ie, String fileName, String fileReferenceType, Map<String, String> attributes) throws DicomException {
		TreeSet whatHasBeenAdded = new TreeSet();
		StringBuffer b = new StringBuffer();
		if (ie == InformationEntity.INSTANCE) {
			b.append(",\'");
		...
		if (ie == InformationEntity.INSTANCE) {
			...
			if (a != null) {
				...
				if (columnName != null && columnType != null && isAttributeUsedInTable(tableName, columnName)
						&& !whatHasBeenAdded.contains(columnName)) {                // do not add same attribute twice to same table
					b.append(",");
					b.append(attributes.get(columnName));
					whatHasBeenAdded.add(columnName);
				}
			}
		}
